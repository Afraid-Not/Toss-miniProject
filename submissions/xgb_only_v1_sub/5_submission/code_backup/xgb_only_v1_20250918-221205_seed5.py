...
def main():
    ...
    if CFG.get("OPTUNA_ON", False) and tune_df is not None:


        Xt = tune_df[xgb_cols].values
        yt = tune_df[target_col].astype(np.int32).values

        # 시계열/일반 분기
        def _build_splits_for_tune():
            if CFG["OPTUNA_USE_TIME_SPLIT"] and (CFG["TIME_COL"] in tune_df.columns):
                order = np.argsort(pd.to_datetime(tune_df[CFG["TIME_COL"]]).values)
                return ("tss", order)
            else:
                return ("skf", None)
