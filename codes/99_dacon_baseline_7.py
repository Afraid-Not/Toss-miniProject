...
CFG = {
    ...
    # Training (GPU/AMP 기준)
    "BATCH_SIZE": 8192,          # 유효 배치 크게
    "MICRO_BATCH": 1024,         # VRAM 부족 시 512로 낮추기
    "EPOCHS": 20,
    "LR": 6e-4,
...
from sklearn.linear_model import LogisticRegression  # (Platt 쓰면 필요)
from sklearn.isotonic import IsotonicRegression      # (Isotonic 쓰면 필요)

def _clip01(p): return np.clip(p, 1e-6, 1-1e-6)
def _logit(p):  p=_clip01(p); return np.log(p/(1-p))
def _sigmoid(z): return 1/(1+np.exp(-z))

# Temperature scaling: p' = sigmoid(logit(p)/T)  (WLL 최소화)
...
# (옵션) Platt: p' = sigmoid(a*z + b)
def fit_platt(y_oof, p_oof):
    z = _logit(np.asarray(p_oof)).reshape(-1,1)
    y = np.asarray(y_oof).astype(int)
    w = np.where(y==1, 0.5/max(1,(y==1).sum()), 0.5/max(1,(y==0).sum()))
...
# (옵션) Isotonic: p' = g(z)
def fit_isotonic(y_oof, p_oof):
    iso = IsotonicRegression(out_of_bounds="clip")
    iso.fit(_logit(np.asarray(p_oof)), np.asarray(y_oof).astype(float))
    return iso
def apply_isotonic(p, iso):
    return _clip01(iso.predict(_logit(np.asarray(p))))

# 앙상블 가중치 탐색 (logit 평균; metric="score"/"ap"/"wll" 중 선택)
def search_w(y, p1, p2, metric="score"):
    y = np.asarray(y).astype(int)
    best, bestw = -1e18, 0.5
    for w in np.linspace(0, 1, 41):
        z = w*_logit(p1) + (1-w)*_logit(p2)
        p = _sigmoid(z)
        ap, wll, sc = compute_metrics(y, p)
        val = {"score": sc, "ap": ap, "wll": -wll}[metric]
        if val > best: best, bestw = val, w
    return bestw
...
def main():
    ...
    train_df['Sin_hour'] = np.sin(2*np.pi*(train_df['hour'] % 24) / 24.0)
    test_df['Sin_hour']  = np.sin(2*np.pi*(test_df['hour']  % 24) / 24.0)
    train_df['Cos_hour'] = np.cos(2*np.pi*(train_df['hour'] % 24) / 24.0)
    test_df['Cos_hour']  = np.cos(2*np.pi*(test_df['hour']  % 24) / 24.0)

    train_df['Sin_dow']  = np.sin(2*np.pi*(train_df['day_of_week']  % 7)  / 7.0)
    test_df['Sin_dow']   = np.sin(2*np.pi*(test_df['day_of_week']   % 7)  / 7.0)
    train_df['Cos_dow']  = np.cos(2*np.pi*(train_df['day_of_week']  % 7)  / 7.0)
    test_df['Cos_dow']   = np.cos(2*np.pi*(test_df['day_of_week']   % 7)  / 7.0)
    ...
    # 새 피처 포함하도록 feature_cols 업데이트
    for c in ['Sin_hour','Cos_hour','Sin_dow','Cos_dow']:
        if c not in feature_cols and c in test_df.columns:
    ...
    # === 여기 추가: LSTM Temperature 보정 (검증 기반) ===
    T_lstm = fit_temperature(y_true_lstm, y_prob_lstm)
    lstm_val_df["p_lstm"] = apply_temperature(lstm_val_df["p_lstm"].values, T_lstm)
    print(f"[CAL] LSTM temperature T={T_lstm:.3f}")

    # a_lstm, b_lstm = fit_platt(y_true_lstm, y_prob_lstm)
    # lstm_val_df["p_lstm"] = apply_platt(lstm_val_df["p_lstm"].values, a_lstm, b_lstm)
    # # 또는
    # iso_lstm = fit_isotonic(y_true_lstm, y_prob_lstm)
    # lstm_val_df["p_lstm"] = apply_isotonic(lstm_val_df["p_lstm"].values, iso_lstm)
    ...
    pred_lstm = apply_temperature(pred_lstm, T_lstm)   # Temperature 기준
    # pred_lstm = apply_platt(pred_lstm, a_lstm, b_lstm)      # (Platt 사용 시)
    # pred_lstm = apply_isotonic(pred_lstm, iso_lstm)   
    ...
    oof_pred_cal  = apply_temperature(oof_pred, T_xgb)
    test_pred     = np.mean(test_pred_folds, axis=0).astype(np.float32)
    test_pred_cal = apply_temperature(test_pred, T_xgb).astype(np.float32)
    print(f"[CAL] XGB temperature T={T_xgb:.3f}")

    # 이후 DataFrame은 보정된 값으로 생성
    xgb_val_df = pd.DataFrame({ "ID": ids_en, "y": y, "p_xgb": oof_pred_cal })
    xgb_df     = pd.DataFrame({ "ID": test_en[ID_COL].values, "p_xgb": test_pred_cal })
    ...
    # ========================= Ensemble (robust weight search) =========================
    def _clip(p): return np.clip(p, 1e-6, 1-1e-6)
    def _logit(p): p = _clip(p); return np.log(p/(1-p))
    def _sigm(z):  return 1/(1+np.exp(-z))

    # metric to pick W: "score" | "ap" | "wll"
    METRIC_FOR_W = "score"   # "wll"로 바꾸면 더 안정적인 편
    ...
    if len(ens_valid) > 0:
        ...
        p1v = np.nan_to_num(_clip(ens_valid["p_lstm"].values.astype(np.float64)), nan=0.5)
        p2v = np.nan_to_num(_clip(ens_valid["p_xgb"].values.astype(np.float64)),  nan=0.5)

        # 진단: 교집합 크기 및 두 모델 상관
        n_int = len(ens_valid)
        r1 = pd.Series(p1v).rank(method="average").to_numpy()
        r2 = pd.Series(p2v).rank(method="average").to_numpy()
        rho = float(np.corrcoef(r1, r2)[0,1])

        # 교집합에서 각 단일 모델 점수(하한선 보장용)
        ap1, wll1, sc1 = compute_metrics(yv, p1v)
        ap2, wll2, sc2 = compute_metrics(yv, p2v)

        # 가중치 수축(prior): 교집합이 작거나 상관 낮으면 더 좋은 단일모델 쪽으로 당김
        prior_w = 1.0 if sc1 > sc2 else 0.0   # 교집합에서 더 좋은 쪽
        # 교집합이 작을수록(√n 분모), 상관이 낮을수록(1-rho 분자) 수축 강하게
        lam = 5.0 * max(0.0, 1.0 - rho) / max(1.0, np.sqrt(n_int))

        def metric_val(y, p):
            ap, wll, sc = compute_metrics(y, p)
            if METRIC_FOR_W == "ap":   return ap, ap, wll, sc
            if METRIC_FOR_W == "wll":  return -wll, ap, wll, sc   # maximize -wll
            return sc, ap, wll, sc

        best = {"key": -1e18, "w": 0.5, "ap": np.nan, "wll": np.nan, "sc": np.nan}
        # 살짝 보수적으로 0.1~0.9 범위에서 탐색 (극단 가중 방지)
        for w in np.linspace(0.1, 0.9, 33):
            pv = _sigm(w*_logit(p1v) + (1-w)*_logit(p2v))
            key, ap, wll, sc = metric_val(yv, pv)
            # 수축 패널티(regularization): (w - prior_w)^2
            key_reg = key - lam * (w - prior_w) * (w - prior_w)
            if key_reg > best["key"]:
                best.update({"key": key_reg, "w": float(w), "ap": float(ap), "wll": float(wll), "sc": float(sc)})

        # 하한선 보장: best가 단일 최고(sc1/sc2)보다 나쁘면 그쪽으로 fallback
        best_single_sc = max(sc1, sc2)
        if best["sc"] + 1e-6 < best_single_sc:
            W = 1.0 if sc1 >= sc2 else 0.0
            ap_e, wll_e, sc_e = (ap1, wll1, sc1) if W == 1.0 else (ap2, wll2, sc2)
            print(f"[ENS ] fallback to single model (W={W:.1f}) | "
                f"Valid | AP {ap_e:.5f} | WLL {wll_e:.5f} | SCORE {sc_e:.5f} | "
                f"n_int={n_int} rho={rho:.3f} lam={lam:.4f}")
        else:
            W = best["w"]; ap_e, wll_e, sc_e = best["ap"], best["wll"], best["sc"]
            print(f"[ENS ] weight* = {W:.3f} | Valid | AP {ap_e:.5f} | WLL {wll_e:.5f} | SCORE {sc_e:.5f} | "
                f"n_int={n_int} rho={rho:.3f} lam={lam:.4f} metric={METRIC_FOR_W}")
    else:
        # 교집합이 없으면 안정적으로 XGB에 가중
        W = 0.0
        ap_e = wll_e = sc_e = np.nan
        print("[ENS ] Valid 교집합 없음 → W=0.0 (XGB 단일) 사용")
    ...
    p1t = np.nan_to_num(_clip(merged["p_lstm"].values.astype(np.float64)), nan=0.5)
    p2t = np.nan_to_num(_clip(merged["p_xgb"].values.astype(np.float64)),  nan=0.5)
    p_blend = _sigm(W*_logit(p1t) + (1-W)*_logit(p2t)).astype(np.float32)
    ...
    ens_score = sc_e if (isinstance(sc_e, float) or np.isscalar(sc_e)) else np.nan
    score_str = (f"{ens_score:.5f}".replace('.', 'p')) if np.isfinite(ens_score) else "NA"
    filename = f"{version}_{SEED}_{score_str}_W{W:.3f}_submit_{ts}.csv"

    out_path = OUT_DIR / filename
    submit.to_csv(out_path, index=False)
    print(f"[SUB ] saved to {out_path} (ENS score: {ens_score if np.isfinite(ens_score) else 'NA'}, W={W:.3f})")

    print(f"[SUM ] LSTM(AP/WLL/SC)={ap_lstm:.5f}/{wll_lstm:.5f}/{sc_lstm:.5f} | "
        f"XGB(AP/WLL/SC)={ap_xgb:.5f}/{wll_xgb:.5f}/{sc_xgb:.5f} | "
        f"ENS(AP/WLL/SC)={'{:.5f}'.format(ap_e) if np.isfinite(ap_e) else 'NA'}/"
        f"{'{:.5f}'.format(wll_e) if np.isfinite(wll_e) else 'NA'}/"
        f"{'{:.5f}'.format(sc_e) if np.isfinite(sc_e) else 'NA'} | W={W:.3f}")
    ...
    with open((SAVE_DIR / "log.txt"), "a", encoding="utf-8") as f:
        ...
        f.write(f"ENS (AP/WLL/SC)={'{:.5f}'.format(ap_e) if np.isfinite(ap_e) else 'NA'}/"
                f"{'{:.5f}'.format(wll_e) if np.isfinite(wll_e) else 'NA'}/"
                f"{'{:.5f}'.format(sc_e) if np.isfinite(sc_e) else 'NA'} | W={W:.3f}\n")
        f.write("="*40 + "\n")
